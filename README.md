...
本仓库将教学内容划分为“例程、题库、课件”三个板块。当前已完成例程库与第一版题库，并建立课件板块的24课次结构：例程按谭浩强《C程序设计（第五版）》第 1–10 章组织，题库按常见考试题型组织，统一使用 MinGW-w64 GCC 验证。课件已完成CW-L01课程引入PPTX与提示词，HTML仍未制作。
...
当前课件板块包含课次目录、课程安排、设计契约和CW-L01课程引入PPTX；其余讲授HTML、图片素材和上机源码骨架仍未制作。
...
- 课件规划：16次讲授课、8次上机课，讲授32学时、上机16学时，共48学时；当前完成度为结构规划24/24、课程引入PPTX 1项、讲授HTML 0/16。
