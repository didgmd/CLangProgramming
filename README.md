...
本仓库将教学内容划分为“例程、题库、课件”三个板块。当前已完成例程库与第一版题库：例程按谭浩强《C程序设计（第五版）》第 1–10 章组织，题库按常见考试题型组织，统一使用 MinGW-w64 GCC 验证。
...
- 程序填空题会自动比较“逐空答案代回后的程序”与完整参考程序；39项旧内容的题目ID交接关系会按源文件语义逐项校验。

## 迁移期边界

- `2023-2024-1/` 与 `2024-2025-1/` 暂时作为只读来源保留。
- 39项待迁内容已由题库接收，例程迁移清单记录了每项对应的稳定题目ID。
- 已完成334/334可追溯性回归；两个旧学年目录及例程迁移清单进入后续独立清理任务，本阶段不删除。
- 题库转录清单、一次性OCR和构题工具在题库验收后删除，日常只保留索引器和校验器。
- 两份教材 PDF 仅作本地参考，已按精确根目录文件名忽略，不得进入 Git。
- 8份原始试卷仅作本地参考，已按精确根目录文件名忽略；Git中只保留规范化后的学生题目。
