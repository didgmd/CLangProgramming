...
本仓库将教学内容划分为“例程、题库、课件”三个板块。当前已完成例程库与第一版题库，并建立课件板块的24课次结构：例程按谭浩强《C程序设计（第五版）》第 1–10 章组织，题库按常见考试题型组织，统一使用 MinGW-w64 GCC 验证。课件已完成CW-L01课程引入PPTX、提示词，以及严格按两个45分钟教学段组织的CW-L01至CW-L16学生端HTML。
...
- 题库：173道，其中选择题65道、判断题14道、程序填空22道、读程序写结果31道、编程题41道。
- 题库内嵌可编译参考程序：63个。
- 8份本地试卷的242个完整题干经复核、规范化和语义去重后形成146道真题型练习；另由39项旧内容构造26道新题。
- 题库内置正常与边界行为场景：79组；41/41道编程题样例和31/31道读程序输出均执行比对。
- 程序审计：94/94道含程序题、116/116个C代码块完成复核；63个完整参考程序全部通过MinGW-w64 GCC 8.1编译。
- 代码排版：多层控制语句同行、控制头与执行语句同行、Tab、非允许超长行、数组下标前空格和同行堆叠普通语句均为0。
- 课件规划：16次讲授课、8次上机课，讲授32学时、上机16学时，共48学时；当前完成度为结构规划24/24、课程引入PPTX 1/16、讲授HTML 16/16、正式上机任务单8/8。学生端HTML均不超过36页；上机任务单采用打印分页展示完整答案，学生仅提交PDF实验报告至超星学习通。
- 原题证据链：8份本地试卷的242道逻辑题已完成逐题映射；原先7组学生端缺口由CW-LAB01至CW-LAB07补齐。来源指纹、逐题状态与学生端证据见[往届试卷考点覆盖](课件/往届试卷考点覆盖.md)及[逐题映射数据](课件/往届试卷题目映射.json)。
...
- 41道编程题均包含输入格式、输出格式、数据范围、样例、评分建议、正常与边界测试和完整参考程序。
- 日常校验器会拒绝单行完整函数、Tab、异常长行、同行堆叠语句、缺失选项或答案、缺失输入输出及样例等退化内容。
- MinGW-w64 GCC验证结果：63个内嵌参考程序、79组确定性行为场景、41道编程题样例和31道读程序输出全部通过；8份上机任务单另以30组学生版行为测试验证。
