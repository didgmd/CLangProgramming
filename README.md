...
本仓库将教学内容划分为“例程、题库、课件”三个板块。当前已完成例程库与第一版题库，并建立课件板块的24课次结构：例程按谭浩强《C程序设计（第五版）》第 1–10 章组织，题库按常见考试题型组织，统一使用 MinGW-w64 GCC 验证。课件目前仅完成规划，尚未制作HTML或PPTX。
...
当前课件板块只包含课次目录、课程安排和设计契约，不包含已完成的HTML、PPTX、图片或上机源码骨架。
...
- 课件规划：16次讲授课、8次上机课，讲授32学时、上机16学时，共48学时；当前完成度为结构规划24/24、正式课件0/16。
